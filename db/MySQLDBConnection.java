...
public class MySQLDBConnection implements DBConnection{
    ...
    public JSONArray searchRestaurants(String userId, double lat, double lon, String term) {
   	 try {
			...
			System.out.println("TEST YELP DATA: "+list.size());
	...
	@Override 
	public JSONArray visitedRestaurants(String userId){
		Set<String> visitedRestaurant_set = getVisitedRestaurants(userId);
		List<JSONObject>visitedRestaurants = new ArrayList<JSONObject>();
		try {
			...
			while (rs.next()){
				...
				if (visitedRestaurant_set.contains(business_id)){
					String businessId = rs.getString("business_id");
					String name = rs.getString("name");
					String categories = rs.getString("categories");
					String city = rs.getString("city");
					String state = rs.getString("state");
					String fullAddress = rs.getString("stars");
					double stars = Double.valueOf(rs.getString("stars"));
					double latitude = Double.valueOf(rs.getString("latitude"));
					double longitude = Double.valueOf(rs.getString("longitude"));
					String imageUrl = rs.getString("image_url");
					String url = rs.getString("url");
					Restaurant restaurant = new Restaurant(businessId, name, categories, city, 
							state, stars, fullAddress, latitude, longitude, imageUrl, url);
					JSONObject obj = restaurant.toJSONObject();
					obj.put("is_visited", true);
					visitedRestaurants.add(obj);
				}	
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return new JSONArray(visitedRestaurants);
	}
	@Override
	...
	public JSONArray itemBasedRecommendRestaurants(String userId) {
		try {
			Set<String> visitedRestaurants = getVisitedRestaurants(userId);
			Set<String> allRestaurants = getAllRestaurants();
			Map<String, String> user_preference = new HashMap<>();
			...
			try {
				...
				while (rs.next()){
					...
					user_preference.put(business_id, user_id);	
				}
			} catch (SQLException e) {
				e.printStackTrace();
			}
			
			Set<String> recommendedBusinessID = ItemBasedRecommendation.getItemBasedRecommendation(visitedRestaurants, 
																				allRestaurants, user_preference);
			if (recommendedBusinessID.size() == 0){
				return null;
			}
			Set<JSONObject> diff = new HashSet<>();
			for (String businessId : recommendedBusinessID) {
				diff.add(getRestaurantsById(businessId, false));
			}
			return new JSONArray(diff);
		} catch (Exception e) {
